...
Gotcha: an auto-created agent can come up in either secret mode — `selective` (no secrets attached, 401s) or `all` (every matching vault secret, including other groups' credentials). See the "check a new agent's secret mode" section of the root CLAUDE.md.
